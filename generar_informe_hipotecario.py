...
def _make_header_cb(logo_path, cot_id, hoy, vigencia, uf_val):
    def draw_header(c, doc):
        ...
        c.drawString(tx, mid + 22, "Simulación de Crédito Hipotecario")
        ...
        c.drawString(tx, mid - 18, "Financiamiento simple, rápido y confiable.")
        ...
        c.drawCentredString(PAGE_W / 2, 10 * mm,
            "Asesorías Loans4B  \u2022  www.loans4b.com  \u2022  Simulación con carácter referencial")
    return draw_header
...
def generate_pdf_hipotecario(data, output_path, logo_path):
    ...
    TIPO_PERS = 'Jurídica' if tipo_raw == 'j' else 'Natural'
    ...
    cuota_data = [[
        Paragraph('<font color="#1A5F9A"><b>Cuota mensual estimada</b></font><br/>'
                  '<font size="8" color="#8FA3B8">Capital + interés + seguros</font>',
    ...
    seg_rows = [
        [Paragraph("<b>Seguro Desgravamen</b>", ST_KV_V),
         Paragraph("Cubre el saldo del crédito ante fallecimiento o invalidez total del deudor, "
        ...
        [Paragraph("<b>Seguro Incendio y Sismo</b>", ST_KV_V),
         Paragraph("Protege la propiedad ante daños estructurales por incendio, temblor o sismo, "
    ...
    # ── Honorarios ─────────────────────────────────────────────────────────
    story += _section("Servicio de asesoría y costos asociados")
    story.append(Paragraph(
        f"Por la asesoría integral en la gestión de este crédito — que incluye tramitación legal, "
        f"coordinación con la entidad financiera, apoyo notarial y acompañamiento durante todo el "
        f"proceso — los honorarios de gestión ascienden a <b>{u(HON_UF)}</b> ({fc(HON_UF)}), "
    ...
    story.append(Paragraph(
        ...
        "Estamos comprometidos a acompañarle en cada etapa del proceso y agradecemos profundamente "
        "su confianza.",
    ...
    sig_data = [[
        Paragraph(f"Atentamente,<br/><br/><b>{ASESOR}</b><br/>"
                  "<font color='#8FA3B8' size='8.5'>Asesor de Crédito Hipotecario</font>",
                  ST_SMALL),
        logo_cell,
    ]]
