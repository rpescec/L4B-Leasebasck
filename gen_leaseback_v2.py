...
def calcular(d):
    ...
    return dict(
        ...
        prepago_rentas=pre, acreedor=acr, cbr=cbr, interes_anio1=ia1,
...
def sim_table_full(r, uf):
    ...
    rows_data = [
        ...
        ('Fee Estructuración Fondo', fmtpm(r['pct_fee_fondo']*100), fmtc(r['fee_fondo']), '', 'normal'),
        ('Gastos Operacionales Totales', '', fmtc(r['descuentos']), fmtuf(r['descuentos']/uf), 'teal_bottom'),
        ('(-) Deuda actual a cancelar', '', fmtc(r['deuda_actual']), fmtuf(r['deuda_actual']/uf), 'normal') if r['deuda_actual'] else None,
    ...
    rows_data = [r2 for r2 in rows_data if r2 is not None]
    ...
    for row in rows_data:
        *cells, kind = row
        if kind == 'hdr':
            ...
            GREEN_DARK2 = colors.HexColor('#0A5E56')
            row_cells = [P(f'<b>{cells[0]}</b>', size=10, color=WHITE)]
            row_cells += [P(f'<b>{c}</b>', size=10, color=WHITE, align=TA_RIGHT if i>0 else TA_LEFT) for i,c in enumerate(cells[1:])]
            table_rows.append(row_cells)
...
def generate_pdf(data, output_path, logo_path):
    ...
    # ── Cuadro de Gastos Operativos + CBR (separado de la simulación) ─────────
    story += [section_header('DETALLE DE GASTOS OPERATIVOS'), SP(6)]
    AW2 = W - 28*mm
    gas_hdr = [
        P('<b>Concepto</b>',    size=8.5, color=WHITE, align=TA_LEFT),
    ...
    gas_cws = [AW2*0.42, AW2*0.17, AW2*0.25, AW2*0.16]
